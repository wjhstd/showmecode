    ...
    protected boolean setSocketOptions(SocketChannel socket) {
        ...
        try {
            ...
            if (channel == null) {
                ...
                // 把SocketBufferHanlder和socket封装成NioChannel
                if (isSSLEnabled()) {
            ...
            // 将NioChannel进一步封装成为NioSocketWrapper,再封装成PollerEvent然后注册到Poller的events队列中去
    ...
    public static class PollerEvent implements Runnable {
        ...
        public void run() {
            if (interestOps == OP_REGISTER) {
                try {
                    // 将PollerEvent中准备就绪的socketChannel注册到Selector。
    ...
    public class Poller implements Runnable {
        ...
        public void run() {
            ...
            while (true) {
                ...
                // any active event.
                // 从Selector选择处就绪的Channel。调用NioEndpoint.processKey()，processKey(）方法中，根据读写事件调用processSocket()处理。
                ...
                if ( close ) {
                    ...
                    if (sk.isReadable() || sk.isWritable() ) {
                        if ( attachment.getSendfileData() != null ) {
                            ...
                            //processSocket()会根据(NioSocketWrapper)socket创建一个SocketProcessor处理器。
                            // SocketProcessor本身实现了Runnable接口。可以作为任务。
                            // 被Endpoint的Executor线程池执行
                            if (sk.isReadable()) {
